...
func _test_bezos_battle_stage() -> void:
	...
	_check(str(stage.get("tutorial_title").text) == "CONTEST", "combat prompt contains only the required action")
	_check("SPACE" in str(stage.get("tutorial_sequence").text), "combat prompt shows the required key")
	...
	_check(str(stage.get("warning_title").text) == "OBJECT", "incoming attack replaces the prompt with OBJECT")
	...
	_check(str(wrong_stage.get_result().get("outcome", "")) == "administrative_defeat", "one wrong action immediately loses the time attack")
	...
	_check(str(timeout_stage.get_result().get("outcome", "")) == "administrative_defeat", "letting the action bar expire immediately loses the time attack")
