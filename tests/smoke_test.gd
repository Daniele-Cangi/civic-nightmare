...
func _test_bezos_battle_stage() -> void:
	...
	_check(is_equal_approx(float(round_maxima.get("legal_shield", 0.0)), 48.0), "Bezos round retains a substantial legal-shield phase")
	_check(is_equal_approx(float(round_maxima.get("bezos_hp", 0.0)), 80.0), "Bezos round durability supports the intended encounter length")
