...
func _test_bezos_battle_stage() -> void:
	...
	stage.setup()
	stage.start()
	_check(stage.get("tutorial_panel").visible, "Bezos battle starts with the guided filing sequence visible")
	_check("CONTEST" in str(stage.get("tutorial_sequence").text), "guided sequence names the first required action")
	_check(stage.get("objection_button").disabled, "OBJECT remains unavailable before an automated action")
	...
	_check(str(stage.get("active_attack")) != "", "two guided CONTEST actions advance to an incoming attack")
	var shield_during_attack := float(stage.get("legal_shield"))
	stage.set("contest_cooldown", 0.0)
	_check(not stage.perform_contest(), "CONTEST cannot be mistaken for defence during an incoming attack")
	_check(is_equal_approx(float(stage.get("legal_shield")), shield_during_attack), "a wrong guided action does not change combat state")
	var hp_before_objection := float(stage.get("citizen_hp"))
	_check(stage.perform_objection(), "OBJECT is accepted during its highlighted timing window")
	stage.process_frame(1.36)
