...
- The six main compounds follow the authored plate's two-column/three-row visual grid. Their facade sprite centers land near world coordinates `x = ±528` and `y = -683 / 0 / 683`; facade-specific logical Y centers compensate for different image heights and doorway offsets. Each specification moves its facade, procedural footprint, entrance, NPC spawn, light, and generated route together; encounter code should not introduce independent world coordinates for those elements.
