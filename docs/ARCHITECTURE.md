...
- The six main compounds follow the authored plate's visual grid: columns `x = -17 / 16` and rows `y = -20 / 0 / 20`. The one-cell horizontal asymmetry compensates for tile-center coordinates around the world origin. Each specification moves its facade center, procedural footprint, entrance, NPC spawn, light, and generated route together; encounter code should not introduce independent world coordinates for those elements.
