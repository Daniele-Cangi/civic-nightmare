...
- `assets/interiors/kremlin_continuity_command_v1.png` and `assets/interiors/elysee_managed_decline_v1.png` are the first authored authority-room backgrounds. `oval_office_room.gd` suppresses its generic visual tile/prop pass when one is present, but retains the shared NPC, exit, lighting, boundary, dialogue, and room-state behavior. Named code-owned barriers follow baked furniture. See [authority interior art direction](AUTHORITY_INTERIOR_ART_DIRECTION.md).
