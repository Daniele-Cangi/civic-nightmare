...
- `assets/landmarks/northern_great_wall_v1.png` replaces Xi's isolated isometric cutout with a 2176×448 transparent northern perimeter. `WorldLandmarkBuilder` anchors it at the north-west world bound, keeps one open central gate aligned to the boulevard, adds two solid wall wings, and retains the former wall asset only as a missing-resource fallback. The hidden bunker now occupies the western margin between rows so it cannot interrupt the northern silhouette. See [Northern Great Wall Art Direction](NORTHERN_GREAT_WALL_ART_DIRECTION.md).
