...
└── scripts/main.gd                 composition root and story coordinator
    ...
    └── encounters/
		...
		├── ufo_observation_problem.gd
		│                           temporal-echo access game and semantic result
...
- The first UFO abduction starts `UfoObservationProblem` before the existing lab scene. The module owns temporal-echo input, observation pads, forgiving scanner invalidation, generated feedback, and its semantic identity result. `main.gd` retains the completed-access bit, records the anomaly through `DossierManager`, and transfers the player into the unchanged Einstein/Zuckerberg encounter. Old saves infer clearance from the raw anomaly event. See [UFO Observation Problem](UFO_OBSERVATION_PROBLEM.md).
